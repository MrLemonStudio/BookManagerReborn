...
class UserActionManager:
    ...
    def __create_user(self, user_name:str, password:str,is_administrator:bool=False)->bool|tuple:
        ...
        if not table_exist:
            self.sqlite.create_table(self.table_name,{"user_name":"text primary key not null","password":"text not null","is_administrator":"boolean not null default false"})
        else:
            pass
        users_data_now=self.sqlite.query(f"select user_name from {self.table_name}")
        for i in users_data_now:
            try:
                ...
                return False
        try:
            self.sqlite.execute_sql(f"insert into {self.table_name}(user_name,password,is_administrator) values(?,?,?)",(user_name,password,is_administrator))
    ...
    def __login_user(self, user_name:str, password:str)->bool|tuple:
        try:
            if not user_name:
                raise UserNameDoesNotMatchException("Please enter user name")
            else:
                pass
            ...
            self.log_file.close()
            return False
        user_name_searched=self.sqlite.query(f"select user_name,password,is_administrator from {self.table_name} where user_name=?",(user_name,))
        try:
        ...
        try:
            ...
            return False
        return True,False
    def lour(self,user_name:str|None, password:str) -> bool:
        return self.__login_user(user_name,password)
